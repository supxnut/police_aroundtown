...
import { Users, UserPlus, Edit3, Trash2, Search, CheckCircle, XCircle, Clock, Calendar, Timer, Plus, Download, User as UserIcon, FileText, Filter } from 'lucide-react';
...
export const AdminUsersPage: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'users' | 'duty' | 'case-report'>('users');
  const [dutyRange, setDutyRange] = useState<'today' | 'week' | 'all'>('today');
  const [users, setUsers] = useState<User[]>([]);
  const [dutyLogs, setDutyLogs] = useState<DutyLog[]>([]);
  const [dutySummary, setDutySummary] = useState({ todayHours: 0, weekHours: 0, totalHours: 0 });
  ...
  const [editingDutyLog, setEditingDutyLog] = useState<DutyLog | null>(null);
  ...
  const fetchDutyLogs = async () => {
    try {
      ...
      if (res.data.success) {
        setDutyLogs(res.data.logs);
        if (res.data.dutySummary) {
          setDutySummary(res.data.dutySummary);
        }
      }
    } catch {
      toast.error('Failed to load duty logs');
    }
  };
  ...
  const handleSubmitDutyModal = async (data: any) => {
    if (editingDutyLog) {
      const res = await api.put(`/duty/${editingDutyLog.id}`, data);
      if (res.data.success) {
  ...
  const getUserQualifiedWorkDays = (userId: number) => {
    ...
    dutyLogs.filter((l) => l.user_id === userId).forEach((l) => {
      if (!l.date) return;
  ...
  const filteredDuty = dutyLogs.filter((log) => {
    const matchesSearch =
    ...
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-800">
        <div>
          <h1 className="text-xl font-bold text-slate-100 flex items-center space-x-2">
            ...
            <span>จัดการบุคลากรและเวลาเข้าเวรปฏิบัติงาน</span>
          </h1>
          <p className="text-xs text-slate-300 mt-1">
            จัดการรายชื่อเจ้าหน้าที่ Discord ID ยศตำแหน่ง และบันทึกเวลาเข้าเวรปฏิบัติงาน
          </p>
        </div>
      ...
      <div className="flex items-center space-x-2 border-b border-slate-800 pb-3">
        ...
        >
          ...
          <span>บันทึกเวลาเข้าเวร ({dutyLogs.length})</span>
        </button>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl flex items-center justify-between shadow-lg relative overflow-hidden group hover:border-slate-700 transition-colors">
              <div className="space-y-1">
                ...
                <div className="flex items-baseline space-x-2">
                  <span className="text-2xl font-black text-rose-400">{dutySummary.todayHours.toLocaleString()}</span>
            ...
            <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl flex items-center justify-between shadow-lg relative overflow-hidden group hover:border-slate-700 transition-colors">
              <div className="space-y-1">
                ...
                <div className="flex items-baseline space-x-2">
                  <span className="text-2xl font-black text-amber-400">{dutySummary.weekHours.toLocaleString()}</span>
                  <span className="text-xs text-slate-500">ชม.</span>
                </div>
              </div>
